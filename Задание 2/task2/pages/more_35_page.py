...
class More_35(Base):
    ...
    def check_text_on_page_more_35(self):
        with allure.step("check text on page more_35"):
            Logger.add_start_step(method="check_text_on_page_more_35")
            self.get_current_url()
            self.read_text_on_page()
            self.assert_word(self.get_text_on_page(), "Not Found")
            self.get_screenshot()
            print("check page more 35 is okay")
            self.driver.back()
            Logger.add_end_step(url=self.driver.current_url, method="check_text_on_page_more_35")
